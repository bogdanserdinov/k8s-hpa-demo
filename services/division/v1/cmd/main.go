...
import (
	...
	service "example/services/division/v1"
)
...
func main() {
	...
	{ // GRPC endpoints
		...
		divisionServer := service.New()
		divisionpb.RegisterDivisionServiceServer(grpcServer, divisionServer)

		logger.Info("starting division grpc server")
	...
	{ // HTTP endpoints.
		...
		logger.Info("starting division healthz server")
	...
	logger.Info("The division-service was terminated with: %v", zap.Error(group.Wait()))
}
